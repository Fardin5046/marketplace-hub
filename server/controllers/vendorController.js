...
// GET /api/vendor/overview — Fix #14: accurate analytics
exports.getVendorOverview = async (req, res, next) => {
  try {
    ...
    const [totalProducts, lowStock, orders, recentOrders, topProducts] = await Promise.all([
      ...
      // Top selling by actual revenue (sum of item price * qty)
      Order.aggregate([
        { $match: { vendor: req.user._id } },
        { $unwind: '$items' },
        { $group: { _id: '$items.product', title: { $first: '$items.title' }, image: { $first: '$items.image' }, totalSold: { $sum: '$items.qty' }, revenue: { $sum: { $multiply: ['$items.price', '$items.qty'] } } } },
        { $sort: { revenue: -1 } },
        { $limit: 4 },
      ]),
    ]);
    ...
    res.json({ profile, totalProducts, lowStock, pendingOrders, totalOrders, totalRevenue, revenueData, recentOrders, topProducts });
  } catch (err) { next(err); }
};

  ...
  try {
    ...
    if (couponCode) {
      ...
      if (coupon && coupon.validTo > new Date() && (!coupon.usageLimit || coupon.usedCount < coupon.usageLimit)) {
        const cartTotal = user.cart.reduce((s, i) => s + (i.product?.price || 0) * i.qty, 0);
    ...
    const orders = [];
    let idx = 0;
    // Track categories to update counts
    ...
    for (const [vendorId, items] of Object.entries(vendorMap)) {
      ...
      // Fix #12: Only COD gets 'pending'; card/apple are 'pending' too (demo - no real payment)
      ...
      const order = await Order.create({
        ...
        couponCode: appliedCouponCode, paymentMethod, deliveryMethod,
        paymentStatus,
        invoiceNumber,
      });
      // Update stock
      for (const i of items) {
        ...
        if (i.product.categorySlug) affectedCategories.add(i.product.category?.toString());
      }
...
exports.updateOrderStatus = async (req, res, next) => {
  try {
    ...
    if (order.orderStatus === 'Delivered' || order.orderStatus === 'Cancelled') {
      return res.status(400).json({ message: `Cannot change status of ${order.orderStatus.toLowerCase()} order.` });
    }
    order.orderStatus = status;

    // When delivered, mark payment as paid (demo: payment confirmed on delivery)

...
export default function Checkout() {
  const { cart, cartTotal, user, refreshCart, appliedCoupon, setAppliedCoupon } = useStore();
  ...
  // Auto-fill shipping from saved default address
  useEffect(() => {
    if (!user) return;
    authApi.getAddresses().then((data: any) => {
      const addresses = data.addresses || [];
      if (addresses.length === 0) return;
      // Pick the default address, or fall back to the first one
    ...
    }).catch(() => { /* no saved addresses, keep defaults */ });
  }, [user]);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      submitting.current = false; // Allow retry on failure
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <aside className="card-surface h-fit p-6 lg:sticky lg:top-24">
          ...
          <div className="mt-4 space-y-3 text-sm">
            ...
            {appliedCoupon && <div className="flex justify-between"><span className="text-muted-foreground">Discount ({appliedCoupon.code})</span><span className="text-success">-{formatPrice(discount)}</span></div>}

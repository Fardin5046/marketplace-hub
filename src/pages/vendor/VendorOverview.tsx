  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Revenue" value={formatPrice(d.totalRevenue || d.profile?.totalRevenue || 0)} delta={18} icon={DollarSign} accent />
        <KpiCard label="Orders" value={String(d.totalOrders || 0)} delta={12} icon={ShoppingCart} />
          ...
          <div className="mt-4 space-y-3">
            {(d.topProducts || []).slice(0, 4).map((p: any, i: number) => (
              <div key={p._id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate">{p.title}</p><p className="text-xs text-muted-foreground">{p.totalSold || 0} sold</p></div>
                <p className="text-sm font-bold">{formatPrice(p.revenue || p.price || 0)}</p>
              </div>
            ))}
          </div>
        </div>

...
export default function ProductForm() {
  ...
  useEffect(() => {
    ...
    if (editing) {
      productApi.getById(id!).then((p: any) => {
  ...
  // Fix #7: Real file upload handler
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    if (files.length === 0) return;
    setUploading(true);
    try {
      const uploaded = await uploadApi.images(files);
    ...
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          {/* Fix #7: Real image upload + URL fallback */}
          <Card title="Images">
            <p className="text-xs text-muted-foreground mb-3">Upload images or add by URL. First image is the main display.</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              {/* Upload button */}
              <button onClick={() => fileRef.current?.click()} disabled={uploading} className="aspect-square rounded-xl border-2 border-dashed border-border hover:border-foreground grid place-items-center text-muted-foreground hover:text-foreground transition disabled:opacity-50">
                <div className="text-center"><Upload className="mx-auto h-5 w-5" /><p className="mt-1 text-xs font-semibold">{uploading ? 'Uploading…' : 'Upload'}</p></div>
              </button>
              {/* URL button */}
              <button onClick={() => { const url = prompt("Image URL:"); if (url) setImages([...images, { url, alt: '' }]); }} className="aspect-square rounded-xl border-2 border-dashed border-border hover:border-foreground grid place-items-center text-muted-foreground hover:text-foreground transition">
                <div className="text-center"><ImageIcon className="mx-auto h-5 w-5" /><p className="mt-1 text-xs font-semibold">Add URL</p></div>
              </button>
            </div>

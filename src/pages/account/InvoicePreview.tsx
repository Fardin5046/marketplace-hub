    ...
    <div>
      ...
      <div className="card-surface p-8 sm:p-12 mx-auto max-w-3xl">
        ...
        <div className="grid gap-6 sm:grid-cols-2 mt-8 text-sm">
          ...
          <div>
            <p className="text-xs uppercase tracking-wider text-muted-foreground">Order details</p>
            <p className="mt-2"><span className="text-muted-foreground">Order: </span><span className="font-semibold">{order.orderNumber}</span></p>
            <p><span className="text-muted-foreground">Status: </span><span className="font-semibold">{order.orderStatus}</span></p>
            <p><span className="text-muted-foreground">Payment: </span><span className="font-semibold capitalize">{order.paymentMethod}</span></p>
            {/* Fix #12: Show payment status on invoice */}
            <p><span className="text-muted-foreground">Payment status: </span><span className={`font-semibold ${order.paymentStatus === 'paid' ? 'text-success' : 'text-warning'}`}>{order.paymentStatus === 'paid' ? 'Paid' : 'Pending'}</span></p>
          </div>
        </div>
        ...
        <div className="mt-6 ml-auto max-w-xs space-y-2 text-sm">
          ...
          {/* Fix #6/#12: Show coupon discount on invoice */}
          ...
          {order.paymentStatus !== 'paid' && (
            <p className="text-xs text-warning font-medium pt-1">⚠ Payment pending — this is a demo order. Payment will be confirmed upon delivery.</p>
          )}
        </div>
...
function Row({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return <div className="flex justify-between"><span className="text-muted-foreground">{label}</span><span className={accent ? "font-semibold text-success" : "font-semibold"}>{value}</span></div>;
}

...
export default function OrderHistory() {
  ...
  const filtered = orders.filter((o: any) => {
    if (active !== "All" && o.orderStatus !== active) return false;
    if (!q.trim()) return true;
    const search = q.trim().toLowerCase();
    if (o.orderNumber?.toLowerCase().includes(search)) return true;
    if (o.invoiceNumber?.toLowerCase().includes(search)) return true;
    if (o._id?.toLowerCase().includes(search)) return true;
    if (o.items?.some((p: any) => p.title?.toLowerCase().includes(search))) return true;
    return false;
  });
  ...
  return (
    <div>
      ...
      <div className="mt-6 relative max-w-md">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search order ID, invoice, or product…" className="h-10 w-full rounded-full border border-border bg-card pl-9 pr-4 text-sm outline-none focus:border-foreground" />
      </div>
      ...
      <div className="mt-6 space-y-3">
        {filtered.length === 0 && (
          <div className="card-surface p-8 text-center text-muted-foreground">
            {q.trim() ? `No orders matching "${q.trim()}"` : "No orders in this category."}
          </div>
        )}
        {filtered.map((o: any) => (

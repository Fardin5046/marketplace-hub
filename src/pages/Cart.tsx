...
export default function Cart() {
  const { cart, updateQty, removeFromCart, cartTotal, user, appliedCoupon, setAppliedCoupon } = useStore();
  ...
  const apply = async () => {
    ...
    try {
      ...
      setAppliedCoupon({ code: d.code, discount: d.discount });
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-3">
          {cart.map((item: any, idx: number) => {
            ...
            return (
              <div key={`${p._id}-${item.size || ''}-${item.color || ''}-${idx}`} className="card-surface flex flex-col sm:flex-row gap-5 p-5">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button onClick={() => removeFromCart(p._id, item.size || '', item.color || '')} aria-label="Remove" className="text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
                  </div>
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center rounded-full border border-border">
                      <button onClick={() => updateQty(p._id, item.qty - 1, item.size || '', item.color || '')} disabled={item.qty <= 1} className="grid h-9 w-9 place-items-center disabled:opacity-30"><Minus className="h-3.5 w-3.5" /></button>
                      <span className="w-8 text-center text-sm font-bold">{item.qty}</span>
                      <button
                        onClick={() => {
                          if (item.qty >= maxStock) { toast.error(`Only ${maxStock} items available.`); return; }
                          updateQty(p._id, item.qty + 1, item.size || '', item.color || '');
                        }}
        ...
        <aside className="card-surface h-fit p-6 lg:sticky lg:top-24">
          ...
          <div className="mt-5 space-y-2.5 text-sm">
            ...
            {appliedCoupon && <Row label={`Discount (${appliedCoupon.code})`} value={`-${formatPrice(discount)}`} accent />}
          ...
          <div className="mt-5">
            {!appliedCoupon ? (
              ...
              <div className="flex items-center justify-between rounded-full bg-success/10 text-success px-4 py-2 text-xs font-semibold">
                ...
                <button onClick={() => setAppliedCoupon(null)}><X className="h-3 w-3" /></button>
              </div>
            )}
          </div>

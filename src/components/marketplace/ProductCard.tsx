...
export function ProductCard({ product: p, layout = "grid" }: { product: any; layout?: "grid" | "list" }) {
  ...
  const rating = Math.round((p.ratingAverage || p.rating || 0) * 10) / 10;
      ...
      <div className="card-surface hover-lift flex gap-5 p-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-1 flex items-center gap-2 text-xs">
            <Star className="h-3 w-3 fill-foreground" />{rating.toFixed(1)} <span className="text-muted-foreground">({reviewCount})</span>
          </div>
    ...
    <div className="card-surface hover-lift group overflow-hidden">
      ...
      <div className="p-4">
        ...
        <div className="mt-2 flex items-center gap-1.5 text-xs">
          <Star className="h-3 w-3 fill-foreground" /><span className="font-semibold">{rating.toFixed(1)}</span>

...
import { LayoutDashboard, Users, AlertOctagon, FolderTree, Bell, Search, LogOut, Menu, X, Shield } from "lucide-react";
      ...
      <div className="flex flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-card/80 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="relative hidden flex-1 max-w-md md:flex items-center">
            ...
            <input placeholder="Search vendors, disputes…" className="h-10 w-full rounded-full border border-border bg-background pl-11 pr-4 text-sm outline-none focus:border-foreground" />
          </div>
          <div className="ml-auto flex items-center gap-3">
            <button className="relative grid h-10 w-10 place-items-center rounded-full hover:bg-muted"><Bell className="h-4 w-4" /><span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-accent" /></button>

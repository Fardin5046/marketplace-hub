...
export const productApi = {
  ...
  getById: (id: string) => request(`/products/id/${id}`),
...
// ─── Cart ─── (Fix #11: variant-aware update/remove)
export const cartApi = {
  get: () => request('/cart'),
  add: (productId: string, qty = 1, size = '', color = '') =>
    request('/cart/items', { method: 'POST', body: JSON.stringify({ productId, qty, size, color }) }),
  update: (productId: string, qty: number, size = '', color = '') =>
    request(`/cart/items/${productId}?size=${encodeURIComponent(size)}&color=${encodeURIComponent(color)}`, { method: 'PATCH', body: JSON.stringify({ qty }) }),
  remove: (productId: string, size = '', color = '') =>
    request(`/cart/items/${productId}?size=${encodeURIComponent(size)}&color=${encodeURIComponent(color)}`, { method: 'DELETE' }),
...
export const couponApi = {
  ...
  update: (id: string, body: any) => request(`/coupons/${id}`, { method: 'PUT', body: JSON.stringify(body) }),
  remove: (id: string) => request(`/coupons/${id}`, { method: 'DELETE' }),
};
...
export const uploadApi = {
  images: async (files: File[]): Promise<{ url: string; alt: string }[]> => {
    const formData = new FormData();
    files.forEach(f => formData.append('images', f));
    const res = await fetch(`${BASE.replace('/api', '')}/api/upload`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });

...
interface StoreContextValue {
  ...
  updateQty: (productId: string, qty: number, size?: string, color?: string) => Promise<void>;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // Fix #6: persist coupon state
  const [appliedCoupon, setAppliedCoupon] = useState<{ code: string; discount: number } | null>(() => read("mk_coupon", null));

  useEffect(() => {
    if (appliedCoupon) localStorage.setItem("mk_coupon", JSON.stringify(appliedCoupon));
    else localStorage.removeItem("mk_coupon");
  }, [appliedCoupon]);
  ...
  // Fix #11: variant-aware update
  const updateQty = useCallback(async (productId: string, qty: number, size = '', color = '') => {
    if (!user) return;
    const d: any = await cartApi.update(productId, qty, size, color);
    setCart(d.cart || []);
  }, [user]);

  // Fix #11: variant-aware remove
  const removeFromCart = useCallback(async (productId: string, size = '', color = '') => {
    if (!user) return;
    const d: any = await cartApi.remove(productId, size, color);
    setCart(d.cart || []);
  }, [user]);
  ...
  }, [user, vendorProfile, cart, wishlist, theme, loading, appliedCoupon, signIn, signOut, setUserData, addToCart, updateQty, removeFromCart, clearCart, toggleWishlist, refreshCart, refreshWishlist]);
